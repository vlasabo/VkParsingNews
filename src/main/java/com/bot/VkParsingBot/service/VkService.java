...
public class VkService {
    ...
    public Map<String, List<String>> getNewsToSendAndSave(User user)
        ...
        var newsFilters = getFilters();
        var newsList = getNews(vk, actor, newsFilters);

        List<String> newsSourcesToSave = new ArrayList<>();
        List<String> resultListToSending = newsList.stream()
                ...
                .peek(item -> newsSourcesToSave.add(getNewsSource(item))) //добавили в лист "отправленные"
                .map(this::getResultNewsToAdd) //в строку
                .collect(Collectors.toList());
        return Map.of(
                "sending", resultListToSending,
                "saving", newsSourcesToSave
        );
    }
